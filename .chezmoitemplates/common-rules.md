...
- 可逆な操作は確認を求めず進めること。可逆とは次をすべて満たす状態を指す: (1) 巻き戻しが実行者自身の操作だけで完結し、他者による取り直し・再同期・作業のやり直しを必要としない（取得できる立場にあるだけの相手は含めず、実際に取得・参照済みの相手を数える）、(2) 影響の到達先が管理境界の内側にとどまり、境界外への公開、境界外での自動実行、境界外への通知・送信のいずれも発火させない、(3) 巻き戻しコストが時間経過や外部の反応で増大しない。管理境界の内側とは指示元の利用者と、その管理下にある private repository の collaborator や社内メンバーを指し、条件(2) の到達先判定にのみ用いる
- 不可逆な操作でも、事前のバックアップ・退避・dry-run によって 3 条件を満たす形へ変換できる場合は可逆として扱ってよい。巻き戻し手段を実行前に用意して作業結果に明記し、退避や検証が取得できない場合は不可逆判定に戻すこと
- 可逆性を確定できないときの既定停止点は、成果物を利用者の管理下に置いたまま提示できる最後の地点とすること。そこまでは進めてよく、停止するのはより可逆な代替経路が無い場合に限ること
- 取り消し手段が存在することを可逆の根拠にしないこと。判定は上記 3 条件だけで行い、1 つでも満たさなければ不可逆として扱うこと
- 共有ブランチへの合流経路とタイミング、レビュー要否、承認者を定める手続規約が存在する場合は可逆性より優先すること。規約は推測せず確認し、正規の確認先が無いか参照できない場合、運用実績などの間接証拠を自走の根拠にしないこと
- permission が通っていることを実行してよい根拠にせず、prompt 回避を目的に経路を書き換えないこと。この規定が緩めるのは操作の実行だけであり、仕様・契約・データ形式などの永続判断や依頼範囲を超えた変更を許可するものではないこと
...
- 委譲は、並列化の利益、context の隔離、専門性、必要な独立性があるときに用いる。既存契約内で同じ owner に閉じる具体作業は親が直接実行してよい。Phase 名や role 名だけを委譲理由にせず、複数モデルの使い分け自体を目的にしないこと
...
- 委譲先の判断は渡した context に基づく限定判断として扱い、親は既存実装・設定・文書・テストへ戻って妥当性を確認し、最終判断・統合・ユーザーへの報告責任を保持すること。委譲先が挙げた提案・推奨・残タスクは、実施・見送り・ユーザー諮問のいずれを採るかを親が項目ごとに選んで理由を付すこと。理由を付さずに未決のまま転送しないこと
...
- artifact を保存するときは初期必須項目として `task`、`phase_or_step`、`created_at` を記す。作業中リポジトリの正規指示または Skill が明示する追加項目にも従う

## Markdown / ADR 運用

- `*.md` ファイルを編集した際は、ファイル全体を見直し、矛盾・重複・ルール漏れがないか確認し、必要なら同じターンで修正すること
- `*.md` ファイルのメタデータは本文に書かず、必ず Front Matter で管理すること
- アーキテクチャ、運用方針、永続設定、複数ファイルにまたがるワークフロー変更などの大きめの変更では、作業リポジトリの正規 ADR 配置に ADR を作成・更新し、作業日時と作業した Agent のモデル名を記載すること
