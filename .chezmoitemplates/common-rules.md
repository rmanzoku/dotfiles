...
- 複数 AI の出力を比較または相互レビューさせる場合は、独立評価が完了するまで作成モデル、provider、agent、所有関係を伏せ、`Report A` / `Report B` などの安定した中立ラベルで渡すこと。親 Agent はラベルと作成者の対応を別途保持し、追跡に必要な場合だけ評価完了後に帰属を復元すること。作成者情報自体を評価する場合は、匿名評価と別 Phase で扱うこと
