...
- `claude-fable-5` / `high` は、長期自律提案またはゼロベース監査で caller が明示指定した場合だけ使う。provider の Fable 固有データ保持境界を受け入れ、hard AI-credit cap を設定し、requested model・effort・credits・elapsed time・artifact を run ごとに記録する。
...
- Fable の長期自律性は利用できるが、機密性と credit 消費を caller が明示的に引き受ける必要がある。
