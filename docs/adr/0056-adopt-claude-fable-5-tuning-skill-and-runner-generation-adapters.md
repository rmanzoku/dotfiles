...
> Fable の task-data authorization と credit floor は [ADR 0069](./0069-normalize-ai-cli-runner-data-boundaries.md) により一部更新された。世代 adapter、30日保持、明示選択、hard cap、暗黙 fallback 禁止は維持する。
...
- 既定のアップグレード先は引き続き Opus 系とし、fable-5-tuning は Fable 5 を明示選択した workflow だけを対象にする。採用前提(明示選択が30日保持の acknowledgment を兼ねること・課金経路の hard cap)の文書化を完了条件に含める。
...
- 世代 adapter の doctrine は opus-5-tuning / fable-5-tuning を正本とし、runner には補正内容の要約だけを持たせる。Fable の実行契約は、明示選択を30日保持の acknowledgment とし、hard AI-credit cap と暗黙 fallback 禁止を維持する。task-relevant input にモデル固有の追加確認を課さない。Fable の長 turn 特性を踏まえ、timeout と credit cap の同時設計を caller checklist に追記する。
