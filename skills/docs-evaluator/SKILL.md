...
- Treat active-doc policy contamination as a first-class risk. If active canonical docs or active skill instructions contain deprecated, temporary, fallback, retired, historical, or override vocabulary as a current option, classify it separately from ordinary stale docs. Do not accept "it is documented as temporary/prohibited" as enough to pass.
...
4. **Classify source-of-truth boundaries**
  ...
  - Check whether temporary or historical content is being used as current policy, whether accepted decisions have not been reflected into canonical docs, whether multiple docs claim canonical authority for the same topic, and whether active docs contain policy contamination from retired vocabulary, temporary/fallback examples, or historical override language.
  - For policy contamination candidates, classify evidence as `allowed-negative-reference`, `migration-map`, `historical-reference`, `search-only-audit-pattern`, `current-policy-contamination`, or `QUESTION`.
...
- Use priority by documentation-system risk: `P0` blocker, `P1` high risk, `P2` meaningful maintainability or AI-readability risk, `P3` optional cleanup. Weight severity by conditioning surface: the same defect is more severe in a document loaded automatically every session (AGENTS.md, CLAUDE.md, entrypoints) than in an on-demand or historical document, because always-loaded text conditions every future agent output.
