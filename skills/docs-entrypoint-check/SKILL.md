...
1. 現在の作業ディレクトリをチェック対象 repo として扱う。
2. `SKILL.md` と同じディレクトリ配下の `scripts/executable_docs_entrypoint_check.py` を実行して JSON を取得する。
3. JSON の `mode` を確認する。
4. `mode=check` なら entrypoint チェック結果を返す。
5. `mode=bootstrap` ならチェック結果に加えて雛形提案を返す。
...
- entrypoint check の観点と制約の詳細は `references/entrypoint_check_prompt.md` を読む。
