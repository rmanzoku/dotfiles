...
- `change-review`: Review a diff/PR/patch in a broader evaluator style. Use findings-first output with severity, file/line references, test integrity, and summary last. During the finding pass, record every issue observed — including uncertain or low-severity ones — with severity and confidence attached; filter and rank when composing the report, not while finding. Check test integrity: assertions weakened or deleted, failing tests removed, tests rewritten to match new behavior, or an implementation changed together with the acceptance tests that gate it, without independent justification. Check cleanup: superseded code paths, schemas, routes, flags, or resolved workaround comments that the diff replaces but leaves active. Stay limited to the diff and directly coupled boundaries; do not drift into unrelated whole-repo critique.
...
- For `change-review`, include `Findings`, `Test Integrity` (missing, weakened, or implementation-coupled tests), `Evidence Coverage`, `Open Questions`, `Checks Run / Not Run`, and `Summary`. Start with findings ordered by severity and include file/line references when available.
