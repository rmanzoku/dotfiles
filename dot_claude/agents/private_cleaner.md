...
- Defaults include the judgment posture the harness already enforces, not only toolchain behavior. Rules restating a conservative default are the most over-retained class — deleting one reads as removing a safeguard — so probe them rather than granting tenure. Their cost is not size alone: a redundant conservative rule biases the reader toward over-stopping.
