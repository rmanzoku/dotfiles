...
- Example: `dot_codex/modify_private_config.toml.tmpl` manages `~/.codex/config.toml`; `private_` keeps the target private, while `modify_` preserves unmanaged runtime sections and merges the declared settings.
